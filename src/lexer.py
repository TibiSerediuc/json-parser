...
class JSONLexer:
    ...
    def handle_default_state(self, char: str):
        ...
        if char in SINGLE_CHAR_TOKENS:
            ...
            self.index -= 1 # So we can iterate over it in the function handler
        else:
    ...
    def handle_unicode_escape_state(seld, char: str):
        ...
        if EscapeHandler.is_valid_hex_digit(char):
            ...
            if len(self.unicode_buffer) == 4:
                unicode_char = EscapeHandler.process_unicode_escape(self.unicode_buffer)
                self.buffer += unicode_char
                self.state = State.STRING
        else:
            raise LexerError(f"Invalid Unicode escape sequence '\\u{self.unicode_buffer}{char}'", self.index)
    ...
    def handle_literal_state(self):
        ...
        token, new_index = LiteralProcessor.process_literal(self.input_string, self.index)
        self.tokens.append(token)
    ...
    def handle_end_of_input(self):
        ...
        if self.state == State.NUMBER:
            ...
            raise LexerError(f"Incomplete literal '{self.buffer}'", len(self.input_string) - len(self.buffer))
        elif self.state == State.ESCAPE:
            raise LexerError("Incomplete escape sequence", len(self.input_string))
        elif self.state == State.UNICODE_ESCAPE:
            raise LexerError(f"Incomplete UNICODE escaoe sequence '\\u{self.unicode_buffer}'", len(self.input_string))
